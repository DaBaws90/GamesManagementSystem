...
public class VentasController {
	...
	public ModelAndView index() {
		...
		logger.regularMessage("FACTURA Nº: " + String.valueOf(timeStampMillis));
...
//	Recibe id del juego que se va a comprar desde la vista detalles del juego, con boton de comprar o, mejor dicho, enlace con apariencia de botón
	...
	public ModelAndView addVentaForm(@PathVariable(name = "id") int id) {
		...
		logger.info("GET", "addVentaForm", "VENTAS_ADD", "VentasController", "VENTA", "INSERTED", "NEW VENTA (Empty)");
		mav.addObject("ventaModel", new VentaModel());
		mav.addObject("juegoModel", juegoService.findById(id));
		mav.addObject("userModel", userService.findByUsername(SecurityContextHolder.getContext().getAuthentication().getName()));
		return mav;
	}
	...
	public ModelAndView addVenta(@Valid @ModelAttribute("ventaModel") VentaModel ventaModel, BindingResult bindingResult, 
		...
		if(bindingResult.hasErrors()) {
			...
			if(ventaService.addVenta(ventaModel) != null) {
				logger.success("VENTA", "INSERTED", ventaModel);
				mav.setViewName("redirect:/juegos/comprasIndex");
				redirectAttributes.addFlashAttribute("insert", 1);
			}
			else {
				logger.unsuccessful("INSERT", "VENTA", ventaModel);
				mav.setViewName("redirect:/ventas/addVenta/" + ventaModel.getJuego().getId());
...
//	public ModelAndView deleteVenta
}
